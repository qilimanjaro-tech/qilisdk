...
.. code-block:: python 
    ...
    backend.execute(DigitalPropagation(ansatz), readout=ReadoutSpec().with_sampling(nshots=1000))
...
.. code-block:: python 
    ...
    backend = QutipBackend()
    result = backend.execute(vqa, readout=ReadoutSpec().with_sampling(nshots=1000))
