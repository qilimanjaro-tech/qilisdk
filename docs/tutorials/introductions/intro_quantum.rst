...
The simplest and most fundamental unit of quantum information is the qubit, which is the quantum version of a classical bit. 
...
where :math:`α` and :math:`β` are complex numbers that represent the probability of the qubit being in the states 0 and 1, respectively.
...
As such, the probability amplitudes must be normalized to ensure that we have a valid probability distribution:

.. math::

   |α|^2 + |β|^2 = 1

When we measure a qubit, the superposition collapses, so we would no longer have a 
superposition of states, but instead we would have either :math:`|0⟩` or :math:`|1⟩`.
If we were to measure the same qubit again immediately after the first measurement, 
...
When we switch to matrix notation we will need to perform the tensor product 
...
.. math::
   ...
   |ψ⟩ = α\begin{pmatrix} 1 \\ 0 \\ 0 \\ 0 \end{pmatrix} + β\begin{pmatrix} 0 \\ 1 \\ 0 \\ 0 \end{pmatrix} + γ\begin{pmatrix} 0 \\ 0 \\ 1 \\ 0 \end{pmatrix} + δ\begin{pmatrix} 0 \\ 0 \\ 0 \\ 1 \end{pmatrix} = \begin{pmatrix} α \\ β \\ γ \\ δ \end{pmatrix}
...
which is a sphere where any point on the surface represents a valid quantum state.
...
Other points on the sphere correspond to different superpositions of 
the :math:`|0⟩` and :math:`|1⟩` states, such as the state :math:`|+⟩` on the equator, 
which is an equal superposition of :math:`|0⟩` and :math:`|1⟩`.
