...
def test_multi_controlled_execution(backend):
    if type(backend) is QiliSim and backend.get_config()["digital_method"] == "statevector_matrix_free":
        pytest.skip(
            "Multi-controlled gates are not currently supported in statevector_matrix_free digital method of QiliSim."
        )
