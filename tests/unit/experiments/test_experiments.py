...
def test_experiment_plotting(monkeypatch):
    ...
    dims2d = [Dimension(labels=["Drive amplitude", "Freq"], values=[np.array([0.1, 0.2]), np.array([1, 2])]), 
    ...
    # 3d should fail
    data3d = np.random.rand(2, 2, 2, 2)
    dims3d = [Dimension(labels=["Dim1", "Freq"], values=[np.array([1, 2]), np.array([10, 20])]), 
