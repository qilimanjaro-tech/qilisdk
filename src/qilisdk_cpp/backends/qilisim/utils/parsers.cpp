...
NoiseModelCpp parse_noise_model(const py::object& noise_model, int nqubits, double atol, const std::vector<double>* step_list) {
    /*
    ...
    */
    ...
    for (auto& py_noise_pass : noise_model.attr("global_noise")) {
        ...
        for (size_t j = 0; j < jump_operators.size(); ++j) {
            ...
            SparseMatrix expanded = (L_qubits == nqubits) ? L : expand_operator(nqubits, L);
