...
class Parameter(BaseVariable):
    ...
    def __le__(self, other: object) -> bool:
        ...
        if isinstance(self_value, complex):
            raise ValueError("Comparing complex expressions is not supported")
        if isinstance(other, (int, float)):
            return self_value <= other
        if isinstance(other, Term) and other.is_parameterized_term():
            other_value = other.evaluate({})
            if isinstance(other_value, complex):
                raise ValueError("Comparing complex expressions is not supported")
            return self_value <= other_value
        if isinstance(other, Parameter):
            if isinstance(other.value, complex):
                raise ValueError("Comparing complex expressions is not supported")
            return self_value <= other.value
...
class Term:
    ...
    def __le__(self, other: object) -> bool:
        if self.is_parameterized_term():
            self_value = self.evaluate({})
            if isinstance(self_value, complex):
                raise ValueError("Comparing complex expressions is not supported")
            if isinstance(other, (int, float)):
                return self_value <= other
