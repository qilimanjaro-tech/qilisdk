...
class QTensorOld:
    ...
    def adjoint(self) -> QTensorOld:
        ...
        """
        return QTensorOld(self._data.getH())

    def trace(self) -> complex:
        # diagonal() returns dense 1D array; summing it is cheap
        return complex(self._data.diagonal().sum())

    def ptrace(self, keep: list[int], dims: list[int] | None = None) -> QTensorOld:
        """
        ...
        If the QTensorOld is a ket or bra, it will first be converted to a density matrix.
        ...
        Raises:
            ValueError: If the product of the dimensions in dims does not match the
                shape of the QTensorOld's dense representation or if any dimension is non-positive.
            ValueError: If the indices in 'keep' are not unique or are out of range.
            ValueError: If the QTensorOld is not a valid density matrix or state vector.
        ...
        if self.is_ket() or self.is_bra():
            ...
            if density >= 0.05 or N <= (1 << 20):  # noqa: PLR2004
                ...
                rho_keep = M @ M.conj().T
                return QTensorOld(csr_matrix(rho_keep))
            ...
            if data:
                ...
                out.sum_duplicates()
                return QTensorOld(out.tocsr())
            return QTensorOld(csr_matrix((Kdim, Kdim)))
        ...
        raise ValueError("The QTensorOld is not a valid state or operator for ptrace().")
    ...
    def norm(self, order: int | Literal["fro", "tr"] = 1) -> float:
        ...
        Args:
            order (int or {"fro", "tr"}, optional): The order of the norm.
                Only applies if the QTensorOld represents a density matrix. Other than all the
    ...
    def unit(self, order: int | Literal["fro", "tr"] = "tr") -> QTensorOld:
        """
        ...
        return QTensorOld(self._data / norm)

    def expm(self) -> QTensorOld:
        """
        ...
        """
        return QTensorOld(expm(self._data.tocsc()))

    def to_density_matrix(self, max_relative_correction: float = _MAX_STATE_CORRECTION) -> QTensorOld:
        """
        Convert the QTensorOld to a density matrix.

        If the QTensorOld represents a state vector (ket or bra), this method
        calculates the corresponding density matrix by taking the outer product.
        If the QTensorOld is already a density matrix, it is returned unchanged.
        The resulting density matrix is normalized.
        If the QTensorOld is a 2**n x 2**n matrix then this method will attempt to convert it into a valid
        density matrix by:
        ...
        Returns:
            QTensorOld: A new QTensorOld representing the density matrix.
        """
        if self.is_scalar():
        ...
        if self.is_ket():
            ...
            if not repaired_state.is_density_matrix():
                raise ValueError("QTensorOld can not be converted into a density matrix (trace≠1 or not Hermitian).")
        ...
        return QTensorOld(rho.data / tr)  # keep it sparse
    ...
    def __add__(self, other: QTensorOld | Complex) -> QTensorOld:
        if isinstance(other, QTensorOld):
            return QTensorOld(self._data + other._data)
        if isinstance(other, Complex) and abs(other) < get_settings().atol:
            return self
        return NotImplemented

    def __radd__(self, other: QTensorOld | Complex) -> QTensorOld:
        return self.__add__(other)

    def __sub__(self, other: QTensorOld) -> QTensorOld:
        if isinstance(other, QTensorOld):
            return QTensorOld(self._data - other._data)
        return NotImplemented

    def __mul__(self, other: QTensorOld | Complex) -> QTensorOld:
        if isinstance(other, (int, float, complex)):
            return QTensorOld(self._data * other)
        if isinstance(other, QTensorOld):
            return QTensorOld(self._data * other._data)
        return NotImplemented

    def __matmul__(self, other: QTensorOld) -> QTensorOld:
        if isinstance(other, QTensorOld):
            return QTensorOld(self._data @ other._data)
        return NotImplemented

    def __rmul__(self, other: QTensorOld | Complex) -> QTensorOld:
        return self.__mul__(other)

    def __repr__(self) -> str:
        r, c = self.shape
        nnz = self._data.nnz
        s = f"QTensorOld(shape={r}x{c}, nnz={nnz}, format='csr')"
        if r * c <= 64:  # noqa: PLR2004
            s += f"\n{self._data.toarray()}"
        return s

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, QTensorOld):
            return NotImplemented
...
def basis_state(n: int, N: int) -> QTensorOld:
    r"""
    ...
    Returns:
        QTensorOld: A QTensorOld representing the \|n⟩'th basis state on a N-size Hilbert space (N=2**num_qubits).
    """
    if not (0 <= n < N):
        raise ValueError(f"n must be in [0, {N - 1}]")
    # one nonzero at (row=n, col=0), value=1.0
    mat = csr_matrix(([1.0], ([n], [0])), shape=(N, 1))
    return QTensorOld(mat)


def ket(*state: int) -> QTensorOld:
    r"""
    ...
    Returns:
        QTensorOld: A QTensorOld representing the multi-qubit ket state.
    """
    if not state:
...
def bra(*state: int) -> QTensorOld:
    ...
    Returns:
        QTensorOld: A QTensorOld representing the multi-qubit bra state.
    """
    return ket(*state).adjoint()


def tensor_prod(operators: list[QTensorOld]) -> QTensorOld:
    """
    Calculate the tensor product of a list of QTensorOlds.

    This function computes the tensor (Kronecker) product of all input QTensorOlds,
    resulting in a composite QTensorOld that represents the combined state or operator.

    Args:
        operators (list[QTensorOld]): A list of QTensorOlds to be combined via tensor product.

    Returns:
        QTensorOld: A new QTensorOld representing the tensor product of the inputs.
...
def expect_val(operator: QTensorOld, state: QTensorOld) -> Number:
    r"""
    ...
    Args:
        operator (QTensorOld): The quantum operator represented as a QTensorOld.
        state (QTensorOld): The quantum state or density matrix represented as a QTensorOld.
...
def reset_qubits(state: QTensorOld, qubits_to_reset: list[int]) -> QTensorOld:
    ...
    Returns:
        QTensorOld: The density matrix with the requested qubits reset to ``|0><0|``.
    """
    ...
    if rest_coo.nnz == 0:
        return QTensorOld(csr_matrix((out_dim, out_dim)))
