...
class Backend(ABC):
    ...
    def _execute_variational_program(
        ...
        def evaluate_sample(parameters: list[float]) -> float:
            param_names = functional.functional.get_parameter_names(parameter_filter=lambda param: param.is_trainable)
        ...
        if len(functional.functional.get_parameters(parameter_filter=lambda param: param.is_trainable)) == 0:
        ...
        optimizer_result = functional.optimizer.optimize(
            ...
            init_parameters=list(
                functional.functional.get_parameters(parameter_filter=lambda param: param.is_trainable).values()
            ),
            bounds=list(
                functional.functional.get_parameter_bounds(parameter_filter=lambda param: param.is_trainable).values()
            ),
