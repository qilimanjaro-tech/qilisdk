...
class PlotlyRenderer:
    ...
    def draw(self, circuit: Circuit, filename: str | None = None) -> None:
        ...
        col_width = self.config["col_width"]
        ...
        if num_cols > 0:
            x_last_center = (num_cols - 1) * col_width
            # left margin
            x_min = -(box_size / 2 + 0.5)
            # right margin
            x_max = x_last_center + box_size / 2 + 0.5
        else:
            x_min = -0.5
            x_max = 0.5
        ...
        if nqubits > 0:
            y_top = 0
            y_bottom = (nqubits - 1) * line_spacing
        else:
            y_top = 0
            y_bottom = 0
        # margin above top wire
        y_min = y_top - 0.5 * line_spacing
        # margin below bottom wire
        y_max = y_bottom + 0.5 * line_spacing
            ...
            for gate in col:
                ...
                if isinstance(gate, M):
                    ...
                    if contiguous and len(qs_sorted) > 1:
                        y_top = qs_sorted[0] * line_spacing
                        y_bottom = qs_sorted[-1] * line_spacing
                        fig.add_shape(
                            type="rect",
                            x0=x - box_size / 2,
                            x1=x + box_size / 2,
                    ...
                    ys = [q * line_spacing for q in (*ctrl, tgt)]
                    y_0, y_1 = min(ys), max(ys)
                    fig.add_shape(type="line", x0=x, x1=x, y0=y_0, y1=y_1, line={"color": "black", "width": line_width})
                    for q in ctrl:
                        add_dot(x, q * line_spacing)
                    if gate.basic_gate.name == "X":
                        add_plus(x, tgt * line_spacing)
                    else:
                        add_box(x, tgt * line_spacing, gate.basic_gate.name)
