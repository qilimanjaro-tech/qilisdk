...
class ExperimentResult(FunctionalResult):
    ...
    def _plot_2d(
        ...
        _MIN_DIMS = 2
        z_override = self.dims_override[2] if apply_z_override and len(self.dims_override) > _MIN_DIMS else None
        x_labels, x_values = x_dim.labels, x_dim.values
    ...
    def plot(
        ...
        *,
        ...
        if fit is not None and fit and not is_amplitude:
            logger.warning(
                "[ExperimentResult] Fitting is only implemented for amplitude plots. "
                "Ignoring fit request for non-amplitude plot."
            )
            should_fit = False
        if n_dimensions == 1:
            self._plot_1d(
